...
export default class EmployeeService {
  ...
  async create(data) {
    ...
    try {
      data.role = await UserRepository.filterIdsInTenant(
        data.role,
  ...
  async update(id, data) {
    ...
    try {
      ...
      const record = await EmployeeRepository.update(
        id,
        data,
        {
          ...this.options,
  ...
  async findById(id) {
    return EmployeeRepository.findById(id, this.options);
  }

  async findAllAutocomplete(search, limit) {
    return EmployeeRepository.findAllAutocomplete(
      search,
      limit,
      this.options,
    );
  ...
  async findAndCountAll(args) {
    return EmployeeRepository.findAndCountAll(
      args,
      this.options,
    );
  }
